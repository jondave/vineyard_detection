...
# Create the graph
G1 = create_detection_graph(geojson_file)

G2 = create_grid_graph(num_rows, num_cols, row_spacing, col_spacing) # Grount truth graph

# Ensure G1 and G2 are created before this point
# Generate adjacency matrices
A1 = nx.adjacency_matrix(G1).todense()  # Convert sparse matrix to dense
...
# Get the node counts
num_nodes_a = A1.shape[0]
num_nodes = num_nodes_b = A2.shape[0]

# Ground truth matrix X_gt: Here it's a simple identity matrix or you can define it as needed
X_gt = torch.zeros(num_nodes, num_nodes)
X_gt[torch.arange(0, num_nodes, dtype=torch.int64), torch.randperm(num_nodes)] = 1

# Check and fix the node count for A1 if necessary
if num_nodes_a < num_nodes_b:
    padding_a = torch.zeros((num_nodes_b - num_nodes_a, A1.shape[1]))  # Padding rows (matching column size of A1)
    A1 = torch.cat([A1, padding_a], dim=0)  # Add padded rows to A1
    padding_a_columns = torch.zeros((A1.shape[0], num_nodes_b - A1.shape[1]))  # Padding columns (matching number of nodes in A2)
    A1 = torch.cat([A1, padding_a_columns], dim=1)  # Add padded columns to A1

# Now both A1 and A2 should have the same number of nodes
n1 = torch.tensor([A1.shape[0]])
n2 = torch.tensor([A2.shape[0]])

print("A1 shape:", A1.shape)  # Should be (num_nodes_b, num_nodes_b)
print("A2 shape:", A2.shape)  # Should be (num_nodes_b, num_nodes_b)
...
plt.subplot(1, 2, 1)
plt.title('Graph 1 Detected Poles')
nx.draw_networkx(G1, pos=pos1, node_color='blue', node_size=100, with_labels=False)
plt.subplot(1, 2, 2)

plt.title('Graph 2 Prior Knowledge')
nx.draw_networkx(G2, pos=pos2, node_color='green', node_size=100, with_labels=False)
plt.savefig('../../images/graph_matching/pytorch/1_grapths.png')
plt.close()
...
for i, node1 in enumerate(pos1.keys()):
    ...
    print(f"Matched node {node1} in G1 to node {node2} in G2")
    ...
    con = ConnectionPatch(
        ...
        axesA=ax1, 
        axesB=ax2, 
        color="green" if G2.has_edge(node1, node2) else "red",
        alpha=0.7
    )
    plt.gca().add_artist(con)
...
# First, initialize positions for all nodes in G2 with default positions (if needed).
...
for i, node1 in enumerate(pos1.keys()):
    ...
    print(f"Aligned node {node1} in G1 to node {node2} in G2")
...
nx.draw_networkx(G2, pos=align_pos2, node_color='green', node_size=100, with_labels=False)
