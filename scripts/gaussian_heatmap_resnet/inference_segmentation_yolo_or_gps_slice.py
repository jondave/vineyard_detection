...
# ==========================================
# 1. TILED INFERENCE FUNCTION
# ==========================================
def predict_whole_image(model, image_path, device, crop_size=(1024, 768), num_classes=4):
    """
    Slides a window across the image, predicts, and stitches the probability map.
    """
    image = Image.open(image_path).convert("RGB")
    original_w, original_h = image.size
    img_np = np.array(image) / 255.0
    
    # Create an empty array for probabilities: (Num_Classes, Height, Width)
    full_probs = np.zeros((num_classes, original_h, original_w), dtype=np.float32)
    # Count map for averaging overlaps
    count_map = np.zeros((original_h, original_w), dtype=np.float32)

    crop_w, crop_h = crop_size
    stride_h = int(crop_h * 0.8) # 20% overlap
    stride_w = int(crop_w * 0.8)

    # Generate grid coordinates
    h_steps = np.arange(0, original_h, stride_h)
    w_steps = np.arange(0, original_w, stride_w)

    # Adjust last steps to ensure we cover the edge
    h_steps[-1] = max(h_steps[-1], original_h - crop_h)
    w_steps[-1] = max(w_steps[-1], original_w - crop_w)
    
    # Remove duplicates if image is smaller than crop or strides align perfectly
    h_steps = np.unique(np.clip(h_steps, 0, original_h - crop_h))
    w_steps = np.unique(np.clip(w_steps, 0, original_w - crop_w))
    ...
    with torch.no_grad():
        for y in h_steps:
            for x in w_steps:
                # Crop
                crop = img_np[y:y+crop_h, x:x+crop_w, :]
                
                # To Tensor
                tensor = torch.from_numpy(crop).permute(2, 0, 1).float().unsqueeze(0).to(device)
                
                # Predict
                output = model(tensor)
                prob = F.softmax(output, dim=1).squeeze(0).cpu().numpy() # (C, H, W)

                # Stitch back
                full_probs[:, y:y+crop_h, x:x+crop_w] += prob
                count_map[y:y+crop_h, x:x+crop_w] += 1

    # Average the overlaps
    full_probs /= count_map[np.newaxis, :, :]
...
if __name__ == "__main__":
    ...
    OUTPUT_FOLDER = "resnet_inference/vineyard_segmentation_paper_1/slice_images/train_resnet101_20260203_135036/inference_results/39_feet/"
    ...
    CROP_SIZE = (1024, 768) # Same as training crop size
    ...
    for img_file in tqdm(image_files):
        ...
        # --- 3. Run Inference ---
        full_probs, width, height = predict_whole_image(model, img_path, DEVICE, CROP_SIZE)
