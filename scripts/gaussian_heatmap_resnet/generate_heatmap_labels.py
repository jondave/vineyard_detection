...
# # --- Base values for mask sizes at the image center ---
# POST_RADIUS_BASE = 50    # Radius for post markers at the center
# ROW_WIDTH_BASE = 70     # Width of the rows at the center
# BLUR_RADIUS_POST = 25    # Base blur radius for post heatmaps
# BLUR_RADIUS_ROW = 35   # Base blur radius for row heatmaps
...
# Camera specifications from DJI Zenmuse H20 Series
camera_specs = {
    "focal_length_mm": 4.5,
...
for feature in geojson_data['features']:
    row_num = feature['properties']['row_number']
    if row_num not in posts_by_row:
        posts_by_row[row_num] = []
    posts_by_row[row_num].append(feature)
...
for image_name in image_files:
    ...
    for post in geojson_data['features']:
        ...
        if pixel_x is not None:
            # post_id = post['properties']['pole_id']
            row_num = post['properties']['row_number']
            ...
            # post_pixel_labels.append({'id': post_id, 'row': row_num, 'coords': (pixel_x, pixel_y)})
            post_pixel_labels.append({'row': row_num, 'coords': (pixel_x, pixel_y)})

    if post_pixel_labels:
        for post in post_pixel_labels:
            row_num = post['row']
            if row_num not in row_pixel_labels:
                row_pixel_labels[row_num] = []
            row_pixel_labels[row_num].append(post['coords'])
            
        for row_num, coords in row_pixel_labels.items():
            coords.sort(key=lambda p: (p[0], p[1]))
            draw_row.line(coords, fill=255, width=row_width)
