...
compass_heading = 170  # Compass heading of rows (degrees clockwise from north, north == 0)
...
for row_idx, expected_count in enumerate(poles_per_row):
    ...
    # Create row feature (LineString)
    row_features.append({
        "type": "Feature",
        "geometry": {
            "type": "LineString",
            "coordinates": [coord.tolist() for coord in interpolated_coords]
        },
        "properties": {
            "type": "row",
            "row_id": row_idx + 1
...
updated_geojson = {
    ...
    "features": row_features + pole_features
}
